...
public:
    ...
    if (params.hardware_info.is_async)
    {
      ...
      async_handler_->init(
        [this, is_sensor_type](const rclcpp::Time & time, const rclcpp::Duration & period)
        {
          ...
          if (
            !is_sensor_type &&
            this->get_lifecycle_id() == lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE)
          {
  ...
  void set_lifecycle_state(const rclcpp_lifecycle::State & new_state)
  {
    ...
    lifecycle_id_cache_ = new_state.id();
  }

  const uint8_t & get_lifecycle_id() const { return lifecycle_id_cache_; }
...
protected:
  ...
  uint8_t lifecycle_id_cache_ = lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
